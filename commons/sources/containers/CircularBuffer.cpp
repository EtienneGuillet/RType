...
#include "exception/NotEnoughDataInCircularBufferException.hpp"
#include "exception/NullPointerException.hpp"

b12software::containers::CircularBuffer::CircularBuffer(size_t size)
    : _bufferSize(size), _readIdx(0), _writeIdx(0), _buffer(nullptr), _full(false), _nbBufferedBytes(0)
{
    if (size == 0) {
        throw exception::CircularBufferException("[CircularBuffer] A buffer can't have a size of 0", WHERE);
    }
    _buffer = new std::uint8_t[size];
}

b12software::containers::CircularBuffer::CircularBuffer(const CircularBuffer &other)
...
b12software::containers::CircularBuffer::~CircularBuffer()
{
    delete [] _buffer;
}

bool b12software::containers::CircularBuffer::write(const void *data, size_t size)
{
    if (data == nullptr)
...
size_t b12software::containers::CircularBuffer::read(void *toWrite, size_t size)
{
    if (toWrite == nullptr)
    ...
    if (_nbBufferedBytes < size)
        throw exception::NotEnoughDataInCircularBufferException(
...
void b12software::containers::CircularBuffer::clear()
{
    _readIdx = 0;
    _writeIdx = 0;
    _full = false;
...
size_t b12software::containers::CircularBuffer::getNbBufferedBytes() const
{
    return _nbBufferedBytes;
}

bool b12software::containers::CircularBuffer::isFull() const
{
    return _full;
}

size_t b12software::containers::CircularBuffer::read(std::string &str, const char *delim, size_t size)
{
    if (size == 0 || delim == nullptr)
        throw exception::CircularBufferException("[CircularBuffer] Delimiter unspecified", WHERE);
    bool found = false;
    size_t msgSize = 0;
...
b12software::containers::CircularBuffer &b12software::containers::CircularBuffer::operator=(const b12software::containers::CircularBuffer &rhs)
{
    if (&rhs == this)
        return *this;
...
size_t b12software::containers::CircularBuffer::getBufferSize() const
{
    return _bufferSize;
}
